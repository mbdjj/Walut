...
//    }
    ...
    func getData(for currency: Currency, base: Currency) async throws -> Currency {
        guard let url = URL(string: "https://api.freecurrencyapi.com/v1/latest?apikey=\(APIKey.key)&base_currency=\(base.code)") else {
            throw NetworkError.invalidURL
        }
...
//        let endDateString = formatter.string(from: endDate)
        ...
        guard let url = URL(string: "https://api.freecurrencyapi.com/v1/latest?apikey=\(APIKey.key)&base_currency=\(baseCode)") else {
            throw NetworkError.invalidURL
        }
        ...
        do {
            let results = try decoder.decode(CurrencyData.self, from: data)
            let currency = Currency(code: currencyCode, rate: results.data.getRate(of: currencyCode))
